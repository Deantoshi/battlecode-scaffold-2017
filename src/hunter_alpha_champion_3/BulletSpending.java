...
/**
 * Eco Greed Supreme - Centralized Bullet Spending
 * 
 * Phase 1 (Rounds 1-800): Pure economy - gardeners and trees only, zero military.
 * Phase 2 (Round 800+): Pivot based on game state.
 *   - If team bullets > 1500: VP rush mode (donate 70% of surplus)
 *   - If team bullets < 1500 or enemies nearby: tank production
 */
public class BulletSpending {
    static RobotController rc;
    static final float BULLET_RESERVE = 300f;
    
    // Phase transition round
    static final int GREED_PHASE_END = 800;
    
    // Phase 2 thresholds
    static final float VP_RUSH_THRESHOLD = 1500f;
    static final float VP_DONATE_RATIO = 0.7f;
    
    // Phase 1 rates
    static final double GARDENER_HIRE_RATE = 0.15;
    static final double TREE_PLANT_RATE = 0.70;
    
    // Phase 2 rates
    static final double GARDENER_HIRE_RATE_PHASE2 = 0.08;
    static final double TANK_BUILD_RATE = 0.12;
        ...
        if (rc.getType() == RobotType.ARCHON) {
            if (isPhase1) {
                ...
                if (vpRushMode) {
                    ...
                    // Tank army mode: hire gardeners who will build tanks
                    Direction dir = randomDirection();
                    if (shouldHireGardenerPhase2(dir)) {
                        rc.hireGardener(dir);
                    }
                    // Some donation even in tank mode
