...
public strictfp class BulletSpending {
     ...
     * Strategy: Prioritize scouts early, delay soldiers until round 500 or enemy gardeners found.
     */
    static void gardenerSpend() throws GameActionException {
        ...
        // Build scouts first - up to MAX_SCOUTS
        ...
        // Check if we should build soldiers (round 500+ or enemy gardener spotted)
        boolean enemyGardenerSpotted = Comms.isEnemyGardenerSpotted();
        boolean shouldBuildSoldiers = round >= 500 || enemyGardenerSpotted;
        
        // Build Soldiers only after scout production and only if conditions met
        if (shouldBuildSoldiers && bullets >= RobotType.SOLDIER.bulletCost && round < 1500) {
            for (int i = 0; i < 8; i++) {
                Direction dir = new Direction(i * (float)Math.PI / 4);
        ...
        // Build Lumberjacks early for tree clearing (limit to 1)
        if (numLumberjacks < 1 && bullets >= RobotType.LUMBERJACK.bulletCost && round < 500) {
            for (int i = 0; i < 8; i++) {
                Direction dir = new Direction(i * (float)Math.PI / 4);
