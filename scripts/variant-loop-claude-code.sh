#!/bin/bash
# variant-loop-claude-code.sh - Variant archetype optimization loop (Claude Code compatible)
#
# Usage: ./scripts/variant-loop-claude-code.sh <bot> <opponent> [map] [max-iterations]
#
# Environment variables:
#   AI_ENGINE    - "claude-code" (default) or "opencode"
#   MODEL        - Model to use (e.g., "sonnet", "opus", "haiku")
#   SKIP_PERMS   - Set to "1" to skip permission prompts (default: 1)
#
...
# AI Engine - default to claude-code
AI_ENGINE="${AI_ENGINE:-claude-code}"

# Model override
MODEL="${MODEL:-}"

# Skip permissions for automation (default: yes)
SKIP_PERMS="${SKIP_PERMS:-1}"
...
if [[ -z "$BOT" || -z "$OPPONENT" ]]; then
    ...
    echo "  AI_ENGINE      'claude-code' (default) or 'opencode'"
    echo "  MODEL          Model to use (sonnet, opus, haiku)"
    echo "  SKIP_PERMS     Set to '1' to skip permission prompts (default: 1)"
    echo ""
    echo "Example:"
    echo "  $0 grok_code_fast_1 copy_bot MagicWood 15"
    echo "  MODEL=opus $0 mybot enemy_bot"
    exit 1
fi
...
[[ -n "$MODEL" ]] && printf '%s\n' "${BLUE}Model:${NC}      $MODEL"
[[ "$SKIP_PERMS" == "1" ]] && printf '%s\n' "${BLUE}Permissions:${NC} Skipped (automation mode)"
echo ""
...
run_agent() {
    ...
    printf '%s\n' "${YELLOW}━━━ Running ${agent_name} agent ━━━${NC}"
    ...
    case "$AI_ENGINE" in
        claude-code|claude)
            ...
            # Build the prompt that invokes the subagent
            local prompt="Use the ${agent_name} agent with these arguments: ${args}"

            printf '%s\n' "${BLUE}Prompt: ${prompt}${NC}"

            # Run Claude Code
            claude "${claude_args[@]}" "$prompt" || exit_code=$?
            ;;
